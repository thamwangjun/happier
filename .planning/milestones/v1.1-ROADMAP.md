...
- [x] **Phase 4: Restructure finalizeCurrentTurn()** — Split into `finalizeCurrentTurn()` (parent, Phase A + B) and `finalizeSubagentTurn()` (subagent, Phase A only); write tests first
- [x] **Phase 5: Verify End-to-End Behavior** — Confirm exactly one ready event fires per parent turn completion; no spurious ready events on subagent completion
