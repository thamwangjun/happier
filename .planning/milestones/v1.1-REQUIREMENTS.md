# Requirements Archive: v1.1 Distinguish Parent vs Subagent Turn Completion
...
- [x] **TURN-02**: Phase A bookkeeping (`activeTaskId = null`, `updateThinking(false)`, transcript flush, diagnostics reset) runs for both parent and subagent completions
- [x] **TURN-03**: Phase B notification (`didFinalizeTurn = true`, `awaitingNextTurnStart = true`, `opts.onReady()`, `scheduleNextMessagePump()`) runs only when `!isSubagent`
...
- [x] **TURN-05**: `messageQueue.flush()` in the `onReady` lambda in `claudeRemoteLauncher.ts` still runs unconditionally — only `readyHandler()` is gated behind `!isSubagent`
- [x] **TURN-06**: A subagent completion followed by a parent completion fires exactly one `ready` event to the relay server and mobile app

### Test Coverage

- [x] **TEST-01**: Test that subagent `task_notification` does not emit a `ready` event
- [x] **TEST-02**: Test that parent `result` after a subagent completion emits exactly one `ready` event
- [x] **TEST-03**: Test that transcript flush (Phase A bookkeeping) runs on both subagent and parent completion paths
