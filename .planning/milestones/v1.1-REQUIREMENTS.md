...
- [x] **SCHEMA-01**: User can set `default: true` in `sessionAgentToolsSettings` to explicitly enable all tools globally (equivalent to current implicit behavior) *(written as `sessionAgentToolsSettingsV1`; renamed per D-06)*
- [x] **SCHEMA-02**: User can set `default: false` in `sessionAgentToolsSettings` to disable all tools by default, enabling opt-in mode *(written as `sessionAgentToolsSettingsV1`; renamed per D-06)*
