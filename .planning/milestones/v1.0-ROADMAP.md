...
**Success Criteria** (what must be TRUE):
  ...
  2. A developer with no `settings.json` (or no `sessionAgentToolsSettings` key) sees all tools available — no behavior change from pre-feature state *(key was `sessionAgentToolsSettingsV1` when written; renamed per D-06)*
  3. A developer with a corrupt or schema-invalid `sessionAgentToolsSettings` value sees all tools available and finds a warning in the daemon log; the daemon does not crash *(key was `sessionAgentToolsSettingsV1` when written; renamed per D-06)*
